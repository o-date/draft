...
VALUES (1, 0001, 001, "The first context of the season. Loamy sand, very herbaceous, characteristic of a typical surface layer.","5YR 3/3",, 3);
```
...
ALTER TABLE `contexts` ADD CONSTRAINT `FK_contexts_Trench` FOREIGN KEY (`Trench`) REFERENCES `contexts` (`Trench`) ON DELETE CASCADE ON UPDATE CASCADE;
```
...
#### Discussion
This kind of work may seem quite daunting at first for those lacking experience at the command line, but it takes time to build up familiarity and confidence with what you're doing. In time, working from the terminal may actually seem _preferable_ to using a graphical user interface; the fact that every action has a purpose, and that the outcomes of every command are predictable and responsive to your actions, makes this work extremely satisfying. At that point, the technology fades to the background and you are left with a series of continuous objectives that build upon each other, and that you are tasked to resolve.

When working as part of a team in a fieldwork setting, the most common challenges are likely to be people-problems rather than tech issues. It is extremely important to be aware of, and to effectively communicate, the limits of what you are capable of accomplishing. Working in fieldwork-based settings is particularly challenging due to various factors.

First, timelines tend to be very rushed, since an entire season of work may last anywhere between 2-8 weeks. Moreover, as a maker you will be learning as you go. It may take you dozens of attempts to make and re-make an effective system before you get it the way you want it, which would be fine under more flexible circumstances. However when working in the field, people may demand that their needs be met in very short time frames. This can be extremely frustrating, for both the maker and the user, since the maker may be pressured to make a shoddy product while the user is made to live with it.

It may also be very difficult to borrow knowledge from others. In places situated off the grid, searching the internet for solutions to a nagging problem may be unfeasible. If internet access is sporadic it may be prudent to download PDF versions of software documentation or to capture screenshots of helpful comments posted on support forums during those rare occasions when it is possible to get online. You may also be one of only a handful of people, or even the only person on the project, with the kinds of skills required to complete your work, which means that asking peers for help may not be helpful.

It is also somewhat common for all work that even remotely relates to a certain conception of computer-based work to be foisted upon one or two individuals who have proven capable of dealing such tasks. That will most certainly include you. Prioritize your time, and show others how to do certain tasks on their own.

When making things, it is important to consider what the thing will be used for, and how it will be used.
