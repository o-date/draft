...
Exit nano by hitting `ctrl + x`, and then pressing the `y` key when asked if you'd like to save the buffer (which means save the file). This should return you to the normal terminal window.
...
Once your changes are made, save the file using `ctrl + x` and then restart the MariaDB service:
...
Now let's close the MariaDB service (using `ctrl + c`) and try logging in as lara:
...
**Pro Tip:** [crontab.guru](https://crontab.guru/) is a great resource for determining the correct syntax for cron schedules.
