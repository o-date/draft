...
Another approach involves writing markdown files, putting them online, and then using a kind of 'helper' file to manage their display as html. In this particular case, we are going to use something called `mdwiki`. [Mdwiki](https://dynalon.github.io/mdwiki/#!index.md) involves a single html file which, when put in the same folder as a series of markdown files, acts as a kind of wrapper to turn the markdown files into pages of a wiki-style website. There is a lot of customization possible, but for now we're going to make a basic notebook out of a mdwiki template.
