...
It doesn't have to be this way. Most archaeological researchers have encountered “bad” data in the past when trying to understand or reconstruct someone’s conclusions from field notes and excavation reports. What makes a dataset unweildy? What would make it better?
...
*It’s difficult to reuse.* The dataset might use inconsistent terminology. The data might be saved as an image, when in fact it's got a structure. The data may already be processed in ways that make further analysis impossible.
...
```{r reptable, message=FALSE}
library(xtable)
reptable <- read.csv("../diagrams/marwick_figure_2.csv", check.names = FALSE)
reptable <- xtable(reptable,
               caption = "Summary of degrees of reproducibility, reproduced from Marwick 2016",
               align = "lp{2cm}p{5cm}p{5cm}p{5cm}p{5cm}")
print(reptable,  comment = FALSE,
      floating = FALSE,
...
Metadata is likewise a critical concept. It's quite literally *data about data*. Some values generated by a machine, like file size or photographic EXIF data, while other *descriptive metadata* is authored by hand. Think of it as the codex that makes the rest of your information useful.
...
Different purposes but both can increase durability of data.
