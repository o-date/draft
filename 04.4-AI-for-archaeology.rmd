...
The Segregation model is one of the oldest and most influential references in the ABM community. However, as the antecedents of the Forest Fire model, the original version of this model is a cellular automaton, where cells are habitat units (*e.g.*, houses, apartments) with three states: empty and two types of inhabitants. Schelling [-@schelling1971] started with an even simpler model, analyzing the mechanisms first in one dimension to then run simulations using 2D grids.
...
The Civil Violence model offers a good opportunity to understand how agent-based simulations may stimulate new insights and help us reason our expectations. Epstein [-@2002, p. 7245-48] comments on the outcomes of this model in light of four aspects:
...
Read the code in `model.py`and `agent.py` carefully. As the first exercise for both practicing and critical discussion, find an aspect of the mesa-Python implementation does not affect simulations, although it was considered by Epstein[-@2002]. This 'phantom code' is probably because this version was left incomplete. HINTS: (1) the *find* tool of your Internet browser can be a powerful ally here; (2) do all parameters have an impact on the model dynamics? Use "`Epstein civil violence.ipynb`" to change parameter values and re-run simulations.
...
As a second more advanced challenge, try to implement the second model presented by Epstein [-@2002]. The second model, referred by Epstein as **_Inter-group violence_**, is an extension where the central authority seeks to suppress communal violence between two groups of citizens.
...
At this stage, we are aiming to set up (*what we think will be*) the core mechanism of our model. We recommend to start with the first intuition that comes to mind. Although its logic structure may be oversimplified, incomplete, or falty after later scrutiny, it would most likely be representative of an informal model that we share with other scholars, given previous contact with the subject. Using an informed guess as a starting strategy helps to avoid the 'blank page panic' while also overthinking and overworking a model version which usufulness lies on its preliminary nature.

The Pond Trade model main idea was inspired on the premisse that the size of the economy asociated with a settlement (as a gross measure of, *e.g.*, population size, volume of material production, built surface) is dependent on the volume of materials exchanged between settlements for economic reasons (named as 'trade'), specifically concerning maritime and fluvial routes as opportunities for transportation. The guiding suggestion was that, because of the schifting nature of trade routes, economy size displays a chaotic behavior as described by [chaos theory](https://en.wikipedia.org/wiki/Chaos_theory) (aparently 'random' oscilations).

![One of the simplest examples of chaotic behavior: double compound pendulum. SOURCE: Catslash (Public domain), from Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Double-compound-pendulum.gif)

Even though the initial inspiration may be theoretically profound, empirically-based, or well-connected to discussion in the academic literature, the initial core mechanism must narrow down to a very simple and inteligible process. In this case, the focus is to represent a positive feedback between economic size and trade volume coming in and out, which can be described as follows:
...
Added to the core mechanisms, we may add a few complementary aspects as we see fit to sufficiently represent the phenomenon. For instance, the first implementation steps in the Pond Trade tutorial addresses the setting of a 'geography' of settlements implicit in the concept of route.

![andros-first-diagram](images/4-4-abm-PondTrade-concept-start.png)

Once a minimum conceptual model is defined, we should use caution when attempting our first implementation. As seen in the previous exercise, a relatively simple model may still correspond to code that can be very difficult to control and understand. Simulation models are very sensitive to the presence of feedback loops. Notice in the tutorial that the positive feedback loops present in the conceptual model are avoided in implementation until the most basic and predictable aspects are properly defined and their behavios verified.
...
When trying to express our initial conceptual model in code, we may encounter several unforseen complications that require fuerther modeling decisions. The emergence of new issues and questions is often due to the 'undefined' or 'informal' nature of the initial approach. For example, if settlement size increase with trade movement, should we consider that it also decreases for other reasons? If not, we would end up having a model of growing settlements racing towards infinity.

At this intermediate stage, we should detect the potential indeterminations affecting the core mechanisms and explicitly include them as parameters or subprocesses according to how important we deem them to be. In respect to modeling details, 'sky is the limit'; however, all additional complexity comes with costs: higher mistake risk, more demanding computational requirements, and greater challanges to validate, document, and comunicate the model.

![Pond Trade conceptual model in step 9](images/4-4-abm-PondTrade-concept-step09.png)
...
![Pond Trade conceptual model in step 13](images/4-4-abm-PondTrade-concept-step13.png)

Elements included in the model up to step 13:

- variables:
  - `settlement`: `sizeLevel`, `currentNumberOfShips`, `potentialNumberOfShips`, `stock`, `culturalVector` (including: three neutral traits, `relativePathCostInLand`, `relativePathCostInPort`, `settlementSizeDecayRate`, `stockDecayRate`, `productionRate`, `frequencyOverQuality`, `traitTransmissionRate`, `mutationRate`)
...
After finishing the tutorial, we encourage to modify/extend/simplify the Pond Trade model following other directions and interests. Another, more technical, exercise is to implement all steps in mesa-Python or any other ABM platform.
