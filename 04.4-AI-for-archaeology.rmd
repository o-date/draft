...
This kind of 'reading' of simulation results are known within the ABM community as 'stylised facts', and it is a common practice when dealing with models with a strong social component.
...
This exercise introduces the student to the process of designing and implementing an original model. We illustrate some of the practical aspects of how to generate the behaviors envisaged when defining a conceptual model. Our particular concern is to offer an example of a systematic, progressive, and test-driven workflow that can help prevent getting lost during the implementation process.

The **_Pond Trade_** model was designed by A. Angourakis in NetLogo to facilitate the learning process regarding ABM for archaeology and history. This model represents mechanisms that links *cultural integration* and *economic cycles* caused by the *exchange of materials* ("trade") between settlements placed in a heterogeneous space ("pond"). The model implementation intentionally includes several aspects that are commonly used in ABM in archaeology and social sciences, such as multiple types of agents, vectors as agent properties (i.e., "cultural vector"), parametric maps (i.e., "procedurally generated")<!-- and interaction with GIS data-->, networks, and the recycling of submodels and algorithms.

The Pond Trade model and all its versions are available for download in [this repository](https://github.com/Andros-Spica/PondTrade). The same repository contains a detailed tutorial explaining all steps ("`README.md`", visible at the main page; **UNDER DEVELOPMENT**). The ".nlogo" files can be executed using a local installation of _**NetLogo**_ (https://ccl.northwestern.edu/netlogo/), which is free and require no registration.
...
1. Initial **conceptual model** (step 0 to step 7).
...
![The model interface in step 9](images/4-4-abm-PondTrade-interface-step07-AStar.png)

2. After including **feedback loops** (steps 8 and 9)
...
3. After including a settlement cultural vector and trait dynamics
...
After finishing the tutorial, we encourage to modify/extend/simplify the PondTrade model following other directions and interests. Another, more technical, exercise is to implement all steps in mesa-Python or any other ABM platform.
