...
b. Schedulle: spectrum between fully-schedulled (processes are initiated in a predetermined order) and event-driven (processes are initiated given a certain input).
ABMs fall somewhere inbetween these two spectra, though normally leanning towards distributed (e.g., using Monte Carlo methods) and fully-schedulled. Applications in several disciplines (see Wikipedia)
...
- Examples
- Mention the cycles of enthusiasm and scepticism, and the tendency to develop closed circles
...
- Modeling is a process of constant iteration. Each stage is an iteration loop that is often repeated several times before jumping into the next step.  The end result of developement and exploration of a model may be relatively stable, often in the form of publications. However, this product potentially feeds a new  modeling effort (and that is actually the main usefulness of models in the long run). 
...
- Walkthrough the implementation in NetLogo
